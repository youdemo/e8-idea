	...
	public void operateToDo(String requestid,String userid) {
		...
		try {
			...
			if (rs.next()) {
				...
				workflowid = Util.null2String(rs.getString("workflowid"));
				if (!"".equals(createrid)) {
					rs1.execute("select * from hrmresource where id=" + createrid);
					if (rs1.next()) {
						creater_loginid = Util.null2String(rs1.getString("loginid"));
					}
					if (!"".equals(workflowid)) {
						rs2.execute("select * from workflow_base where id=" + workflowid);
						if (rs2.next()) {
			...
			while (rs3.next()) {
				...
				String sendAccount = "";//新增sendaccount推送  add by dingji at 20190417
				if (!"".equals(receiverid)) {
					rs1.execute("select * from hrmresource where id=" + receiverid);
					if (rs1.next()) {
						receiver_loginid = Util.null2String(rs1.getString("loginid"));
						sendAccount = receiver_loginid;
						String belongto = Util.null2String(rs1.getString("belongto"));
						if(belongto.length() > 0){
							rs1.execute("select * from hrmresource where id=" + belongto);
							if (rs1.next()) {
...
//				String flownodename = "";
//				rs1.execute("select nodename from workflow_nodebase where id= "+ node_id);
//				if (rs1.next()) {
...
//				}
				...
				String appurl =  "/client.do?method=toDetail&loginid="+receiver_loginid+"&timestamp="
						+timestamp+"&loginTokenFromThird="+hexSHA1("weaver"+receiver_loginid+timestamp)
							+"&url="+URLEncoder.encode("/mobile/plugin/1/view.jsp?detailid="+ requestid+"&fromofs=1","UTF-8");
				...
				//for(String whiteuser:whitelist.split(",")){
					//if(receiver_loginid.equals(whiteuser)){
						if ("0".equals(isremark)) {
							...
							Map<String, String> dataMap = buildDataMap(syscode,
									...
									receiver_loginid, receivedatetime, requestlevel ,sendAccount);
					...
					//		writeLog("PostWorkflowInf(158)");
							...
							Map<String, String> dataMap = buildDataMap(syscode,
									...
									createdatetime, receiver_loginid, receivedatetime,requestlevel ,sendAccount);
							...
							Map<String, String> dataMap1 = buildDataMap(
									...
									receiver_loginid,requestlevel,sendAccount);
...
//									rs1.execute("select * from hrmresource where id=" + receiverid1);
//									if (rs1.next()) {
...
//								rs1.execute("select nodename from workflow_nodebase where id= "+ node_id1);
//								if (rs1.next()) {
...
//							}
	...
	public void operateZXToDo(String requestid,int id){
		...
		writeLog("===征询改待办operateZXToDo--start" + " " + requestid + "===");
		String syscode = Util.null2String(Prop.getPropValue("HYJK_TOdoConfig", "syscode"));
		...
		try {
			...
			if (rs.next()) {
				...
				requestlevel = Util.null2String(rs.getString("requestlevel"));
				if (!"".equals(createrid)) {
					rs1.execute("select * from hrmresource where id="
							+ createrid);
					if (rs1.next()) {
						creater_loginid = Util.null2String(rs1.getString("loginid"));
					}
					if (!"".equals(workflowid)) {
						rs2.execute("select * from workflow_base where id=" 
								+ workflowid);
						if (rs2.next()) {
							workflowname = Util.null2String(rs2.getString("workflowname"));
						}
			...
			while(rs3.next()){
				...
				String flownodename = "";
				rs1
						.execute("select nodename from workflow_nodebase where id= "
								+ node_id);
				if (rs1.next()) {
					flownodename = Util.null2String(rs1.getString("nodename"));
				}
				...
				String appurl =  "/client.do?method=toDetail&loginid="+receiver_loginid+"&timestamp="+timestamp+"&loginTokenFromThird="+hexSHA1("weaver"+receiver_loginid+timestamp)+"&url="+URLEncoder.encode("/mobile/plugin/1/view.jsp?detailid="+ requestid+"&fromofs=1","UTF-8");
	...
	public void NoReadToRead(String requestid,String userid){
		...
		writeLog("===未读改已读NoReadToRead--start" + " " + requestid + "=== "+userid);
		String syscode = Util.null2String(Prop.getPropValue("HYJK_TOdoConfig", "syscode"));
		String whitelist = Util.null2String(Prop.getPropValue("bmd", "whitelist"));
		if (requestid == null || "".equals(requestid)) {
			return;
	...
	public void deleteoonlyUserToDo(String requestid,String userid) {
		...
		writeLog("===删除用户流程传输统一代办库deleteoonlyUserToDo---start" + " " + requestid + "===");
		String syscode = Util.null2String(Prop.getPropValue("HYJK_TOdoConfig", "syscode"));
		try {
			RecordSet rs1 = new RecordSet();
	...
	public void deleteUserToDo(String requestid,String userid) {
		...
		writeLog("===删除用户流程传输统一代办库deleteUserToDo---start" + " " + requestid + "===userid===="+userid);
		String syscode = Util.null2String(Prop.getPropValue("HYJK_TOdoConfig", "syscode"));
		String whitelist = Util.null2String(Prop.getPropValue("bmd", "whitelist"));
		try {
			RecordSet rs = new RecordSet();
	...
	public void deleteToDo(String requestid) {
		...
		writeLog("===删除流程传输统一代办库deleteToDo---start" + " " + requestid + "===");
		String syscode = Util.null2String(Prop.getPropValue("HYJK_TOdoConfig", "syscode"));
		String whitelist = Util.null2String(Prop.getPropValue("bmd", "whitelist"));
		try {
			RecordSet rs = new RecordSet();
...
//				String viewtype = "1";
	...
	public Map<String, String> buildDataMap(String syscode, String flowid,
			...
			String receiver, String receivedatetime,String requestlevel,String sendAccount) {	
		...
		dataMap.put("sendAccount", sendAccount);
		return dataMap;
	}
	...
	public Map<String, String> buildDataMap(String syscode, String flowid,
			String requestname, String workflowname, String nodename,
			String receiver,String requestlevel,String sendAccount) {
		Map<String, String> dataMap = new HashMap<String, String>();
		dataMap.put("syscode", syscode);
		dataMap.put("flowid", flowid);
		dataMap.put("requestname", requestname);
	...
	public Map<String, String> buildDataMap(String syscode, String flowid,
			...
			String receivedatetime , String requestlevel,String sendAccount) {
		Map<String, String> dataMap = new HashMap<String, String>();
		dataMap.put("syscode", syscode);
		dataMap.put("flowid", flowid);
		dataMap.put("requestname", requestname);
	...
	//	String oaAddress = "";
		...
		rs.execute("select password,loginid from hrmresource where id="+ id);
		if (rs.next()) {
			password = Util.null2String(rs.getString("password"));
			loginid = Util.null2String(rs.getString("loginid"));
		}
		para = "/" + gotoPage + "?requestid=" + requestid + "&fromofs=1#" + loginid + "#" + password;
