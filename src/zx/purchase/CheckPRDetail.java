...
public class CheckPRDetail {
	...
	public String getCanCheckForWw(String checkid){
		String id=checkid.split("_")[0];
		String rqid=checkid.split("_")[1];
		RecordSet rs = new RecordSet();
		int count = 0;
		String sql="select count(b.id) as count from formtable_main_155 a,formtable_main_155_dt1 b where a.id=b.mainid and b.prdtid='"+id+"' and a.requestid not in("+rqid+")";
		rs.execute(sql);
		if(rs.next()){
			count = rs.getInt("count");
		}
