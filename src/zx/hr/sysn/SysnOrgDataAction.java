...
public class SysnOrgDataAction extends BaseCronJob{
	...
	public String sysOrg() {
		...
		String result = getORGSapData(aa.format(new Date()), "X", "000000", "235959", "1");
	...
	public String sysPerson() {
		SimpleDateFormat aa = new SimpleDateFormat("yyyyMMdd");
		String result = getORGSapData(aa.format(new Date()), "X", "000000", "235959", "3");
		...
		try {
			...
			if ("S".equals(MSG_TYPE)) {
				...
				for (int index = 0; index < jsonArr.length(); index++) {
                    ...
                    String PKTXT = jo.getString("PKTXT");// 员工子组
                    String ATEXT = jo.getString("ATEXT");//工资范围
                    String ICNUM_1 = jo.getString("ICNUM_1");//身份证号码
                    String ICNUM_2 = jo.getString("ICNUM_2");//护照号码
                    String ICNUM_3 = jo.getString("ICNUM_3");//台胞证
                    String DAT02 = jo.getString("DAT02");//入SMEC日期
                    String DAT03 = jo.getString("DAT03");//年资核定日
                    String BTEXT = jo.getString("BTEXT");//直间接
                    String BEGDA = jo.getString("BEGDA");//合同开始日期
                    String CTEDT = jo.getString("CTEDT");//合同结束日期
                    String SYDQR = jo.getString("SYDQR");//试用期到期
                    String MC_STEXT = jo.getString("MC_STEXT");//职位名称
                    String RUFNM = jo.getString("RUFNM");//别名
					String SLART = jo.getString("SLART");//学历文本
					String PCODE = jo.getString("PCODE");//政治面貌
					String PCALL = jo.getString("PCALL");//
...
//					hrb.setTelephone(telph);
					...
					hrb.addCusMap("field10", RUFNM);
                    hrb.addCusMap("field11", PTEXT);//员工组
                    hrb.addCusMap("field12", PKTXT);//员工子组
                    hrb.addCusMap("field13", ATEXT);//工资范围
                    hrb.addCusMap("field14", ICNUM_1);//身份证号码
                    hrb.addCusMap("field15", ICNUM_2);//护照号码
                    hrb.addCusMap("field16", ICNUM_3);//台胞证
                    hrb.addCusMap("field17", DAT01);//Report Date
                    hrb.addCusMap("field18", DAT02);//入SMIC日期
                    hrb.addCusMap("field19", DAT03);//年资核定日
                    hrb.addCusMap("field20", BTEXT);//直间接
                    hrb.addCusMap("field21", BEGDA);//合同生效日
                    hrb.addCusMap("field22", CTEDT);//合同截止日

					hrb.addCusMap("field26", JOBCL_T);//职类
					hrb.addCusMap("field27", ANSVH);//职等
					hrb.addCusMap("field28", SYDQR);//试用期到期
					hrb.addCusMap("field29", MC_STEXT);//职位名称
					hrb.addCusMap("field30", PCODE);//政治面貌
					hrb.addCusMap("field31", SLART);//学历文本
					hrb.addCusMap("field32", PCALL);//学历文本
	...
	public String sysJobtitle() {
		writeLog("同步岗位开始");
		SimpleDateFormat aa = new SimpleDateFormat("yyyyMMdd");
		String result = getORGSapData(aa.format(new Date()), "X", "000000", "235959", "2");
