package jsd.financialcontrol.serviceimpls;
...
public class CheckBudget implements Action  {
		...
		try {
			...
			if("1".equals(status_re)) {
				...
				for(int i =0;i<arr.length();i++) {
					...
					String strsq = "update "+tablename+"_dt4  set kyys = '"+accumulativeTotal+"',overfee ='"+over_re
							+"',action ='"+action_re+"',msg='"+msg_re+"',status='"+status_re+"' where id = '"+map.get(rowNum)+"'";
					logs.writeLog("updatestrsq----"+strsq);
					rs.execute(strsq);
				}
			}else {
	...
	public String getKYYSParameter(String rid,String tablename,Map <String,String> map) {
		...
		String sqlstr =  "select m.id as mid,dt.* from  "+tablename +" m join "+tablename+"_dt4 dt on  m.id=dt.mainid   where m.requestid = '"+rid+"'";
		RecordSet  rs = new RecordSet();
		int a = 1;
		rs.execute(sqlstr);
		while(rs.next()) {
			...
			bxje = Util.null2String(rs.getString("hsje"));
			String  kmmc = gu.getKMFieldVal("uf_kindeedata", "fname", "fnumber", fcostid+"' and flag = '1");//科目名称
...
//			组装项目号，对应明细id
	...
	public String getKYYS(String method_kyys,String appToken_kyys,String st) {
		...
		new BaseBean().writeLog("获取可用预算数据-传参---"+json.toString());
