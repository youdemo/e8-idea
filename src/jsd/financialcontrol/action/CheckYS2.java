...
* @version 1.0  差旅报销校验预算是否超标
*/
public class CheckYS2  implements Action  {
...
//		
		...
		int con3 = 0;
		int con4 = 0;
		String sql = "select count(dt.id) as con  from "+tablename+" m join "+tablename+"_dt6 dt  on  m.id = dt.mainid where dt.overfee = '1' and (dt.action='2' or dt.action='1') and m.requestid = '"+requestid+"'";
		rs.execute(sql);
		if(rs.next()){
			con1 = rs.getInt("con");
		}
		
		sql = "select count(dt.id) as con  from "+tablename+" m join "+tablename+"_dt6 dt  on  m.id = dt.mainid where dt.action='2' and m.requestid = '"+requestid+"'";
		rs.execute(sql);
		if(rs.next()){
			con2 = rs.getInt("con");
		}
		sql = "select count(dt.id) as con  from "+tablename+" m join "+tablename+"_dt6 dt  on  m.id = dt.mainid where  dt.action='1' and m.requestid = '"+requestid+"'";
		rs.execute(sql);
		if(rs.next()){
			con3 = rs.getInt("con");
		}
