...
//		String sfyfp = "";//是否有发票413
		...
		sql = "select kdkjehj,clfzje,sfyfp,fkyh,jdkmdm,(select lastname from hrmresource where id =isnull((select xm from uf_byjedb where id =a.byjk ),0)) as bgyname," +
				"(select workcode from hrmresource where id =isnull((select xm from uf_byjedb where id =a.byjk ),0)) as bgrgh,ybje,ytje,sfsybyj,zjkdh_new," +
				"jine,id,djbh,wsjezh,sezh,qkr,gh,(select lastname from hrmresource where id=a.qkr) as name,qkdwwb,(select account_name from uf_gsb where id=a.gsb) " +
				" as account_name,gsdm,qkdw,sezh,ccrq,ccjssj from "+tableName+" a where requestid="+requestid;
		rs.executeSql(sql);
		if(rs.next()){
...
//			clfzje = Util.null2String(rs.getString("je"));
...
//		}
		...
		int flh = 1;
		sql = "select a.bmmc as deptid ,(select departmentname from hrmdepartment where id=a.bmmc) as bmmc,je,gsdm from "+tableName+"_dt7 a where mainid='"+mainid+"'";
		rs.executeSql(sql);
		while(rs.next()) {		
...
//			String hsxmdf = pu.getFZHS(dfkm);
			...
			mapStr.put("KMBM", jdkmdm);//科目编码
			mapStr.put("KMMC", fkyhname);//科目名称//科目名称
