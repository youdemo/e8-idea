...
/**
 * @author 作者  张瑞坤
 * @version 创建时间：2019-2-25 下午2:19:59
 * 类说明
 */
public class CreateJJFPZAction implements Action{
...
//		String sfyfp = "";//是否有发票413
		...
		sql = "select kdkjehj,sfyfp,je,fkyh,jdkmdm,(select lastname from hrmresource where id =isnull((select xm from uf_byjedb where id =a.byjk ),0)) as bgyname,(select workcode from hrmresource where id =isnull((select xm from uf_byjedb where id =a.byjk ),0)) as bgrgh,ybje,ytje,sfsybyj,zjkdh_new,jine,id,djbh,wsjehj,sezh,qkr,gh,(select lastname from hrmresource where id=a.qkr) as name,qkdwwb," +
				"(select account_name from uf_gsb where id=a.gsb)  as account_name,gsdm,qkdw,sezh from "+tableName+" a where requestid="+requestid;
		rs.executeSql(sql);
		if(rs.next()){
			djbh = Util.null2String(rs.getString("djbh"));
			wsjezh = Util.null2String(rs.getString("wsjehj"));
...
//			sql = "select  "+wsjezh+"+"+sezh+" as fpjezh";
			sql = "select "+wsjezh+"+"+sezh+"+"+kdkjehj+" as fpjezh";
//		}else{
//			sql = sql+jes+"+"+sezh+" as fpjezh";
//		}
		rs.execute(sql);
		if(rs.next()) {
...
//		}
		...
		int flh = 1;
		sql = "select a.bmmc as deptid ,(select departmentname from hrmdepartment where id=a.bmmc) as bmmc,je,bmdm from "+tableName+"_dt3 a where mainid='"+mainid+"'";
		rs.executeSql(sql);
		while(rs.next()) {		
