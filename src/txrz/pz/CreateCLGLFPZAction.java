...
/**
 * @author 作者  张瑞坤
 * @version 创建时间：2019-2-25 下午2:20:41
 * 类说明
 */
public class CreateCLGLFPZAction implements Action{
	...
	public String execute(RequestInfo info) {
		...
		String requestname = info.getRequestManager().getRequestname();
		...
		sql = "select kdkjehj,sfyfp,je,fkyh,jdkmdm,(select lastname from hrmresource where id =isnull((select xm from uf_byjedb where id =a.byjk ),0)) as bgyname," +
				"(select workcode from hrmresource where id =isnull((select xm from uf_byjedb where id =a.byjk ),0)) as bgrgh,(select  u.gbdq from uf_gbdq u where u.id = a.gbdq) as gbdq, (select u.gbdqbm from uf_gbdq u where u.id = a.gbdq) as gbdqbm,ybje,ytje,sfsybyj,(select gysmc from uf_gysjcb where id = a.gys) as gysname,(select gysbm from uf_gysjcb where id = a.gys) as PortalLoginid," +
				"zjkdh,jine,id,djh,wsjezh,sezh,qkr,gh,(select lastname from hrmresource where id=a.qkr) as name," +
		...
		if(rs.next()){// 
			...
			zjkdh_new = Util.null2String(rs.getString("zjkdh"));
...
//			fylxname = Util.null2String(rs.getString("fylxname"));
		...
		log.writeLog("fkyhname---"+fkyhname);
		if("".equals(wsjezh)) {
			wsjezh = "0";
		}	
		if("".equals(sezh)) {
