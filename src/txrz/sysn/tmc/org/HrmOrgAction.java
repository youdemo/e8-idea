package txrz.sysn.tmc.org;

import txrz.sysn.tmc.util.TmcDBUtil;
...
//			}
	...
	public ReturnInfo operJobtitle(HrmJobTitleBean hjt){
		...
		RecordSet rs = new RecordSet();
		String sql = "";
		sql = "select id from HrmJobTitles where jobtitlename='"+hjt.getJobtitlename()+"' ";
		rs.executeSql(sql);
		int id = 0;
		if(rs.next()){
			id = rs.getInt("id");
...
//		}
	...
	private String updateCustom(String tableName,Map<String,String> mainMap,Map<String,String> updateMap){
		...
		RecordSet rs = new RecordSet();
		// 判断值是否存在，不存在话插入
		String sql = "select count(1) as ct from " + tableName + where;
		rs.executeSql(sql);
		int flag = 0;
		if(rs.next()){
			flag = rs.getInt("ct");
		...
		while (it.hasNext()) {
			...
			String tmp_1_str = updateMap.get(tmp_1);
			if (tmp_1_str == null)
				tmp_1_str = "";
