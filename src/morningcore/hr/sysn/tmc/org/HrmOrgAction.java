package morningcore.hr.sysn.tmc.org;

import java.util.HashMap;
import java.util.Iterator;
import java.util.Map;

import morningcore.hr.sysn.tmc.util.TmcDBUtil;
...
public class HrmOrgAction {
	...
	public ReturnInfo operJobtitle(HrmJobTitleBean hjt){
		...
		BaseBean log = new BaseBean();
		ReturnInfo ri = new ReturnInfo();
		String jobtitlename = hjt.getJobtitlename();
		RecordSet rs = new RecordSet();
		String sql = "";
		...
		if(id < 1){
			...
			mapStr.put("jobtitlemark", hjt.getJobtitlename());
			...
			rs.executeSql("select id from HrmJobTitles where jobtitlename='"+jobtitlename+"' ");
			if(rs.next()){
				id = rs.getInt("id");
			}
			if(id < 1){
	...
	public ReturnInfo operResource(HrmResourceBean hrb){
		...
		if(jobFlag == 0){
			...
			sql = "select id from HrmJobTitles where jobtitlename='"+hrb.getJobtitleCode()+"'";
			rs.executeSql(sql);
			if(rs.next()){
		...
		if(id < 1){
			...
			mapStr.put("isAdAccount",hrb.getIsAdAccount());
			mapStr.put("ldapid",hrb.getLdapid());
//			int s = 0;
