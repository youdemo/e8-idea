...
public class CreateLXLLXML {
	public String CreateXML(String begindate,String enddate,String yjcbzx,String scr,String rqids) {
		...
		int entryid = 1;
		sql="select f.fylx,d.fykmfl,a.cbzx,(select cbzxbmmc from uf_cbzx where id=a.cbzx) as cbzxmc,c.pp,sum(isnull(c.rmbje,0)) as rmbje from "+tablename_lxly+" a,"+tablename_lxly+"_dt1 c,workflow_requestbase b,uf_NPP d,uf_materialDatas e,uf_cbzx f where a.requestid=b.requestid  and a.id=c.mainid and e.wllx=d.id and c.wlmc_2 = e.id and a.cbzx=f.id and b.currentnodetype=3 and a.requestid in ("+rqids+")" +
