 ...
 * desc 每周发送上周的流程延期邮件
 */
public class SendWorkFLowDelayReportMail20200807 extends BaseCronJob {
    public void execute() {
    ...
    public boolean getSendData(String startDate,String endDate,String emails){
        ...
        sql	+="   and b.nodeid in (select distinct jdid from uf_workflow_csmt)" +
                ...
                "   group by a.workflowid,c.departmentid ) t order by workflowid asc,departmentid asc";
        rs.execute(sql);
        int xh =1;
        int allcount =0;
        while(rs.next()){
    ...
    public boolean sendmail(String mails, List<Map<String,String>> list,String showdata,int allcount){
        ...
        content.append("&nbsp;&nbsp;&nbsp;&nbsp;各位领导，附件为本期的OA流程超时数据，涉及到的相关部门总监请输出改善对策。");
        content.append("<br/>");
        content.append("&nbsp;&nbsp;&nbsp;&nbsp;快速响应。");
        String excelName ="OA红榜（"+showdata+"）.xlsx";
        String newFileName = "";
        try {
        ...
        InputStream is = null;
        try {
            is = getExcelFile(list,allcount);
        } catch (Exception e) {
            log.writeLog("生成附件失败",e);
            try {
                is.close();
        ...
        boolean result = sm.sendMiltipartHtml(mailfrom,mails,"","","OA红榜",content.toString(),3,filenames,filecontents,"3");
