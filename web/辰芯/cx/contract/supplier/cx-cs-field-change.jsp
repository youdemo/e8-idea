...
<HTML>
	...
	String sql = "select "+fieldname+",GFMC from uf_khzsjxjgyjm where id="+customid;
	rs.executeSql(sql);
	if(rs.next()){
		...
		<div >
			...
			<%	
					...
					<%
							...
							String isMustInput="1";
							String browserUrl = "/systeminfo/BrowserMain.jsp?url=/interface/CommonBrowser.jsp?type=browser."+cfb.getZdyanbs();
					%>
						<wea:item>
						<brow:browser name='<%=showfieldname%>'
									  viewType='0'
									  browserValue='<%=fieldValue2%>'
									  isMustInput='<%=isMustInput%>'
									  browserSpanValue="<%=biu.getZDYFieldValue(cfb,fieldValue2)%>"
									  hasInput='true'
									  linkUrl=''
									  completeUrl=''
									  width='20%'
