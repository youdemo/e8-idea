...
if @type = '0'
	begin
		select  @zfje2= isnull(sum(isnull(dt.jysje,0.00)),0.00) from formtable_main_268  m join formtable_main_268_dt1 dt on  m.id = dt.mainid join workflow_requestbase w
		 on m.requestId=w.requestid where w.currentnodetype>0 and w.currentnodetype<3 and dt.sxkz = @uid and ( CONVERT(varchar, m.yfjkd) = '' or m.yfjkd is null );
		...
		on dt.mainid=m.id   where  w.currentnodetype>0 and w.currentnodetype<3 and dt.sxkz= @uid;
		...
		on dt.mainid=m.id   where  w.currentnodetype>=3 and dt.sxkz= @uid and m.requestid  
  ...
  not null and CONVERT(varchar,yfjkd) <> '' )A OUTER APPLY( SELECT yfjkd = N.v.value('.', 'varchar(100)') FROM A.[yfjkd].nodes('/root/v') N(v))B );
		...
		select  @zfje6= isnull(sum(isnull(dt.jysje,0.00)),0.00) from formtable_main_268  m join formtable_main_268_dt1 dt on  m.id = dt.mainid join workflow_requestbase w
		 on m.requestId=w.requestid where w.currentnodetype>0  and dt.sxkz = @uid and  CONVERT(varchar, m.yfjkd) <> '' and m.yfjkd is not null ;
		 ...
		 on m.requestId=w.requestid where w.currentnodetype>=3 and dt.sxkz = @uid and  CONVERT(varchar, m.yfjkd) <> '' and m.yfjkd is not null ;
	...
	begin
		...
		select  @zfje2= isnull(sum(isnull(dt.jysje,0.00)),0.00) from formtable_main_268  m join formtable_main_268_dt1 dt on  m.id = dt.mainid join workflow_requestbase w
		 on m.requestId=w.requestid where w.currentnodetype>=3 and dt.sxkz = @uid ;
