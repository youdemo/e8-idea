alter function f_get_wprate(@i_wpid varchar(100),@i_rq varchar(100),@i_type varchar(10))
     ...
     declare
		...
		if @i_type=0
			begin
			   ...
			   if @v_maxid <> ''
			   begin
					select @v_result=isnull(sl,0) from formtable_main_87_dt1 where id=@v_maxid
			   end
			end
			...
			begin
			   ...
			   if @v_maxid <> ''
			   begin
					select @v_result=isnull(sl,0) from uf_lyktzb_dt1 where id=@v_maxid
			   end
			end
