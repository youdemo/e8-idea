create or replace function f_fna_getdkje(i_fpid in varchar2,
                                         ...
                                         i_xm   in varchar2,
                                         i_type in varchar2)
--i_fplx 发票类型  i_je 金额 i_bxje 报销金额 i_sl 税率 i_type 类型 0可抵扣 1不可抵扣 i_xm项目类型
  ...
  if i_type = '0' then
      ...
      if i_fplx = '10' or i_fplx = '8' then
        ...
        select count(1)
         ...
         where seller like '%滴滴出行%'
           and id = i_fpid;
        if v_count > 0 then
          select round(nvl(i_bxje, 0) / (1 + (i_sl / 100)) * (i_sl / 100),
                       2)
            into v_result
            from dual;
        else
    ...
    if i_xm = '33' or i_xm = '46' then
      ...
      if i_fplx = '10' or i_fplx = '8' then
        ...
        if v_count > 0 then
          select round(nvl(i_je, 0) / (1 + (i_sl / 100)) * (i_sl / 100), 2) -
                 round(nvl(i_bxje, 0) / (1 + (i_sl / 100)) * (i_sl / 100),
                       2)
            into v_result
            from dual;
        else
